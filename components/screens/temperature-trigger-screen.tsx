...
interface TemperatureTriggerScreenProps {
  onBack: () => void;
  onContinue?: () => void;
}

export function TemperatureTriggerScreen({ onBack, onContinue }: TemperatureTriggerScreenProps) {
  const [temperature, setTemperature] = useState(20);
  const [comparison, setComparison] = useState<"<" | "=" | ">">(">");

  const minTemp = -50;
  const maxTemp = 50;
  const tempPercent = ((temperature - minTemp) / (maxTemp - minTemp)) * 100;
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 py-4 bg-white">
        ...
        <h1 className="text-lg font-semibold text-gray-900">Temperature</h1>
        <div className="w-6" />
      </div>
          ...
          <div className="px-6 pb-6">
            <div className="text-center mb-6">
              <div className="flex items-start justify-center">
                <span className="text-7xl font-light text-gray-900">{temperature}</span>
                <span className="text-3xl mt-2 text-gray-900">°C</span>
              </div>
            </div>
            ...
            <div className="relative h-2 mb-2">
              ...
              <div
                ...
                style={{ left: `calc(${tempPercent}% - 12px)` }}
              />
            </div>
