...
interface HumidityTriggerScreenProps {
  onBack: () => void;
  onContinue?: () => void;
}

const humidityLevels = ["Dry", "Comfortable", "Moist"] as const;

export function HumidityTriggerScreen({ onBack, onContinue }: HumidityTriggerScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 py-4 bg-white">
        ...
        <h1 className="text-lg font-semibold text-gray-900">Humidity</h1>
        <div className="w-6" />
      </div>
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                  selected === level ? "border-primary bg-primary" : "border-gray-300"
                }`}
              >
                {selected === level && (
              ...
              <span className={`font-medium ${selected === level ? "text-gray-900" : "text-gray-700"}`}>
                {level}
              </span>
            </button>
          ))}
