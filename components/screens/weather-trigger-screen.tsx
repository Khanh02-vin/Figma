...
interface WeatherTriggerScreenProps {
  onBack: () => void;
  onContinue?: () => void;
}

const weatherOptions = ["Sunny", "Cloudy", "Rainy", "Snowy", "Hazy"] as const;

export function WeatherTriggerScreen({ onBack, onContinue }: WeatherTriggerScreenProps) {
  const [selected, setSelected] = useState<string>("Rainy");
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 py-4 bg-white">
        ...
        <h1 className="text-lg font-semibold text-gray-900">Weather</h1>
        <div className="w-6" />
      </div>
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                  selected === weather ? "border-primary bg-primary" : "border-gray-300"
                }`}
              >
                {selected === weather && (
              ...
              <span className={`font-medium ${selected === weather ? "text-gray-900" : "text-gray-700"}`}>
                {weather}
              </span>
            </button>
          ))}
